...
_LABEL_NAMES = ["intentional_stop", "free_forward", "free_left", "free_right", "blocked"]
...
class ChartRenderer:
    def render(self, snap: ScenarioSnapshot, metrics: dict, path: str | Path) -> None:
        ...
        per_step_counts = {name: [] for name in _LABEL_NAMES}
        for step in snap.timesteps:
            step_counts = {name: 0 for name in _LABEL_NAMES}
            for agent in step["agents"]:
                lbl = agent["transition_label"]
                if lbl is not None and 0 <= lbl < len(_LABEL_NAMES):
                    step_counts[_LABEL_NAMES[lbl]] += 1
            for name in _LABEL_NAMES:
                per_step_counts[name].append(step_counts[name])

        # Subplot 1: Bar chart of transition-label counts
        counts = [metrics.get(f"{name}_count", 0) for name in _LABEL_NAMES]
        ax1.bar(_LABEL_NAMES, counts)
        ...
        ax2.stackplot(
            ...
            labels=["free_forward", "free_left", "free_right", "intentional_stop", "blocked"],
            alpha=0.8,
        )
